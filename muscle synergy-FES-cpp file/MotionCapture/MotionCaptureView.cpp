...
CMotionCaptureView::CMotionCaptureView()
{
	...
	f = fopen("data2.txt", "w");
	ff = fopen("data3.txt", "w");
	fff = fopen("datadecdc.txt", "w");
	ffff = fopen("data5.txt", "w");
}
...
void CMotionCaptureView::OnInitialUpdate()
{
	...
	///xsensWraper.setDeviceID("00B43037", 1);
	//xsensWraper.setDeviceID("00B43009", 1);
...
void CMotionCaptureView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (lsChar == 'Q')
	{
		fprintf(f, " test is strating");
		fprintf(ff, " test is strating");
		fprintf(fff, " test is strating");
		fprintf(ffff, " test is strating");
...
void CMotionCaptureView::State() {
	...
	// 이니셜 컨택트 지정 
	for (int i = 1; i < 20; i++) {
		jhtmp_1[i - 1] = jhtmp_1[i];
	}

	jhtmp_1[19] = R_Anglesag;


	windownum = 2;
	front_1 = 0; back_1 = 0;
	for (int i = 19-2*windownum; i < 19-windownum; i++) {
		front_1 = front_1 + jhtmp_1[i];
	}


	for (int i = 20 - windownum; i < 20; i++) {
		back_1 = back_1 + jhtmp_1[i];
	}

	mid_1 = windownum * jhtmp_1[19 - windownum];
...
void CMotionCaptureView::update_tmp() {
	...
	fprintf(f, " this is IC");
	fprintf(ff, " this is IC");
	fprintf(fff, " this is IC");
	fprintf(ffff, " this is IC");
